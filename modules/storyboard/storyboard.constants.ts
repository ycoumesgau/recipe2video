...
export const FOOD_VIDEO_PROMPT_RULES = [
  ...
  "The standardized outro does not compress logical scenes, but seedance-segments.json still requires logicalSceneIds to contain at least one non-empty string. Use a placeholder id such as scene-outro (no matching logical-scenes.json row is required). Never emit an empty logicalSceneIds array on the outro segment.",
